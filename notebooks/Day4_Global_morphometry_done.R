...
master_subinfo <- read.table("data/sample_mrishare_subinfo.csv", header=TRUE, sep=",", dec=".")
head(master_subinfo)
...
eTIVRange = seq(min(female_dat$FS6_eTIV), max(female_dat$FS6_eTIV), 50000)

#data frame for mean age
f_pred_dat_mean_age <- data.frame(eTIV=eTIVRange,
                                  eTIV_c=eTIVRange - mean(female_dat$FS6_eTIV),
                                  MeanAge=mean(female_dat$Age),
                                  Age_c=mean(female_dat$Age_c))
#data frame for low age
f_pred_dat_low_age <- data.frame(eTIV=eTIVRange,
                                 eTIV_c=eTIVRange - mean(female_dat$FS6_eTIV),
                                 LowAge = mean(female_dat$Age) - 2*sd(female_dat$Age))
f_pred_dat_low_age$Age_c <- f_pred_dat_low_age$LowAge - mean(female_dat$Age)

#data frame for high age
f_pred_dat_high_age <- data.frame(eTIV=eTIVRange,
                                  eTIV_c=eTIVRange - mean(female_dat$FS6_eTIV),
                                  HighAge = mean(female_dat$Age) + 2*sd(female_dat$Age))
f_pred_dat_high_age$Age_c <- f_pred_dat_high_age$HighAge - mean(female_dat$Age)

# Now predict the CT using the 3 DF above
# Test with most complex model you are willing to test
lm1_CT_f_noSqAge <- lm( FS6_DKT_T_ct ~ Age_c + eTIV_c + Age_c:eTIV_c, data=female_dat)
lm1_CT_f_noSqAgestep <- step(lm1_CT_f_noSqAge, scope=list(lower=FS6_DKT_T_ct ~ Age_c, upper=lm1_CT_f_noSqAge))
summary(lm1_CT_f_noSqAgestep)

f_CT_pred_mean_age <- predict(lm1_CT_f_noSqAgestep, f_pred_dat_mean_age)
f_CT_pred_low_age <- predict(lm1_CT_f_noSqAgestep, f_pred_dat_low_age)
f_CT_pred_high_age <- predict(lm1_CT_f_noSqAgestep, f_pred_dat_high_age)
...
par(mar=c(4, 4, 4, 4) + 0.1)
plot(female_dat$FS6_eTIV, female_dat$FS6_DKT_T_ct,
     col=adjustcolor("coral1", alpha.f = 0.5), pch=16 , cex=1.3,
     main="Cortical thickness change versus total intracranial volume in females",
     xlab="Total intracranial volume", ylab="Mean cortical thickness (mm)")

# Lines of prediction
lines(eTIVRange, f_CT_pred_mean_age, col="firebrick4", lwd=2)
lines(eTIVRange, f_CT_pred_low_age, col="firebrick4", lty=3, lwd=2)
lines(eTIVRange, f_CT_pred_high_age, col="firebrick4", lty=2, lwd=2)
